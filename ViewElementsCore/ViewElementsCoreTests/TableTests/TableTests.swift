...
class TableTests: XCTestCase {
    ...
    func testTableSubscripts() {
        ...
        var table = Table(sections: [s0, s1])
        ...
        table[IndexPath(row: 1, section: 1)] = row(num: 999)

        XCTAssert(table[IndexPath(row: 1, section: 1)] == row(num: 999))
    }
}
